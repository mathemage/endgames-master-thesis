...
In~contrast to solving games by linear programming, we can approximate \acrlong{ne} using various \emph{learning algorithms}:
\begin{description}
  \item [\acrfull{cfr}] is a~\emph{self-playing} algorithm.
    ...
    The average strategy over all iterations converges towards a~\acrshort{ne} for the game, and is thus used as the final strategy for the play.
    (\cite{Zinkevich2007regret})

  \item [\acrfull{mccfr}] (\cite{Johanson2012efficient})

  \item [\cfrplus] (\cite{Tammelin2015solving})
\end{description}
...
For an~overview, consult also (\cite{Bosansky2013solving}).
