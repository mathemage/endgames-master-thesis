...
\noindent
\begin{itemize}
  \item \emph{Sequence} $s_i(h)$ denotes player~$i$'s history of~actions, along the (unique) path from the root to node~$h$.
    Set~$S_i$ is the set of~all valid sequences of~$i$.

  \item The \emph{realization probability} of sequence~$s \in S_i$ under behavior strategy~$\sigma_i$ is $\sigma _i [s] = \prod_{a \in s} \sigma_i(a)$.

  \item The \emph{realization plan} of some $p$'s mixed strategy $\mu_p$ is a~mapping $x\colon S_p \to \mathbb{R}$ defined by $x(s) = \sum _{\textrm{pure strategy } \sigma _p} \mu _p (\sigma _p) \cdot \sigma _p [s]$.
    We usually write $x$ resp. $y$ for realization plan of player~$1$ resp. player~$2$.

  \item Mixed strategies $\sigma _p, \sigma' _p \in \Sigma _p$ are \emph{realization equivalent} if they reach any game state with equal probabilities, against an~arbitrary strategy of the opponent.
