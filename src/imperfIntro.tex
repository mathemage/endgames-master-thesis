...
\]
The right-hand side is therefore the unit vector $e = (1, 0, \dots, 0) ^\top$ and the matrix~$E$ is sparse, with $\Onot(\lv \I_1 \rv)$ non-zero elements.
Analogously for $Fy = f$.

The \emph{sequence-form payoff matrix}~$A \in \mathbb{R} ^{\lv S_1 \rv \times \lv S_2 \rv}$ for player~$1$ is at every sequence $s \in S_1$ and $s' \in S_2$ described as
\[
  A _{s,s'} = \sum _{\substack{z \in Z\\ s_1(z) = s,\; s_2(z) = s'}} u_1(z) \; \pi_c (z),
\]
Player~$2$'s sequence-form payoff matrix is $-A$, because of~the zero-sum property.
\clearpage
\todo
