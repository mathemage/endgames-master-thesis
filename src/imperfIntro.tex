...
\[
  ...
  = x ^\top A y
\]
Hence, a~best-response realization plan $x$ against any realization plan~$y$ maximizes $1$'s utility $u_1(\sigma) = x ^\top (Ay)$.
It is, by also using (\ref{eq:seq-constraints}), a~solution to \acrshort{lp} 
\begin{equation}
