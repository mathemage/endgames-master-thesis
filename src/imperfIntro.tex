...
\begin{itemize}
  ...
  \item Recall that a~mixed strategy $\mu_i$ is a~probability distribution over (all) pure strategies of player~$i$.
    ...
    It expresses the probability of~following sequence~$s$ under distribution~$\mu_i$.
\end{itemize}
Commonly, letter~$x$ (resp. $y$) is used for realization plans of player~$1$ (resp. player~$2$).
The necessary conditions are
\begin{equation}
  \label{eq:seq-cond-init}
  x(\emptyset) = 1, \quad y(\emptyset) = 1
\end{equation}
and
\begin{align}
  \label{eq:seq-cond-children}
  \begin{split}
    x(s_1(I)) &= \sum _{a \in A(I)} x(s_1(I \cdot a)), \\
    y(s_2(I)) &= \sum _{a' \in A(I)} y(s_2(I \cdot a'))
  \end{split}
\end{align}
