...
\noindent
\begin{itemize}
  \item \emph{Sequence}~$s_i(h)$ describes player~$i$'s history of~actions, i.e. edges along the (unique) path from the root to node~$h$.
  ...
  \item Recall that a~mixed strategy $\mu_i$ is a~probability distribution over (all) pure strategies of player~$i$.
    A~\emph{realization plan} $x\colon S_i \to \R$ is then prescribed as
    \[ x(s) = \sum _{\textrm{pure strategy } \sigma} \mu_i(\sigma)\; \pi^{\sigma}[s] \]
    In other words, it expresses the probability of~following sequence~$s$ under distribution~$\mu_i$.
    Commonly, letter~$x$ (resp. $y$) is used for realization plans of player~$1$ (resp. player~$2$).
\end{itemize}
