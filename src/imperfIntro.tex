...
A~possible solution is to~use the \emph{sequence form}.
...
\begin{itemize}
  \item A~\emph{sequence} $s_i(h)$ is player~$i$'s history of~actions, on the (unique) path from the game-tree root to node~$h$.
    Set~$S_i$ denotes all valid sequences of~$i$.

  \item The \emph{realization probability} of a~sequence $s_p$ of player~$p$ under a~behavior strategy $\sigma _p$ is $\sigma _p [s] = \prod_{a \in s} \sigma _p(a)$.
