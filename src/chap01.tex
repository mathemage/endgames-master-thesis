...
\begin{enumerate}
  \item Generate local game trees:
    ...
    \begin{enumerate}[(a)]
      \item \emph{pruning rules} (e.g. pruning moves dominated by other moves)%
  ...
  \item Evaluate local games as mathematical games.
    ...
    Although a~solved position is reached, (\cite{Muller1995computer}) notes that a~``suicidal'' opponent may later cause great troubles, by endangering his immortal stones.
  ...
  \item Select a~move in the resulting sum-game.
  \item Translate the chosen move into a~corresponding Go position.
\end{enumerate}
...
The results are stored in a~database of~local positions.
...
(\cite{Muller1995computer}) also implements a~version with lower memory requirements.
