...
The innovative aspect of~(\cite{BurchJohansonBowling13}) consists of~two contributions:
\begin{itemize}[(i)]
  \item a~technique to re-solve imperfect-information subgames, which is guaranteed not to increase exploitability of~the resulting full-game strategy,
  \item a~new off-line solving algorithm called \emph{\acrfull{cfr-d}}, which computes an~error-bounded \acrshort{ne} approximation.
    It achieves this by decomposing and independently analyzing subgames.
\end{itemize}
