...
\begin{thm}[unexploitable subgame re-solving]
  ...
  If $u_i^r (\sigma) \le u_i^r (\sigma^*)$ (the worst-case utility improved in~the subgame), then at~every node $h \in H$ we also have $u_i^h (\sigma) \le u_i^h (\sigma^*)$ (the worst-case utilities improved everywhere else).
\end{thm}
\begin{proof}
  ...
  If $T_h$ does not contain $S$, then the best responses $\sigma_{-i}$ and $\sigma^*_{-i}$ are identical within $T_h$, and thus, $u_i^h (\sigma) = u_i^h (\sigma^*)$.
  If, on~the other hand, $T_h$ does contain $S$, we will prove the inequality by~mathematical induction on the height of~$T_h$.
