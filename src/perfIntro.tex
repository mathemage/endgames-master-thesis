...
\begin{proof}
  ...
  \begin{align*}
    ...
    + \sum _{z \in Z \setminus S} \pi^{\sigma}\!(z) \,u_i(z)
    %
    \stackrel{\text{(\ref{eq:opt-val-of-subgame})}}{\ge} \pi^{\sigma}\!(r) \,u_i^r(\sigma_S)
    + \sum _{z \in Z \setminus S} \pi^{\sigma}\!(z) \,u_i(z) \\
    %
    &= \sum _{z \in Z \cap S} \pi^{\sigma}\!(z) \,u_i(z)
    + \sum _{z \in Z \setminus S} \pi^{\sigma}\!(z) \,u_i(z)
...
no opponent can exploit the newly computed sub-strategy by adjusting his play (either in~the trunk or the subgame).
\begin{thm}[unexploitable subgame re-solving]
  ...
  If $u_i^r (\sigma) \le u_i^r (\sigma^*)$ (the worst-case utility does not decrease in~the subgame), then at~every node $h \in H \setminus S$ we also have $u_i^h (\sigma) \le u_i^h (\sigma^*)$ (the worst-case utilities outside the subgame do not decrease).
\end{thm}
\begin{proof}
  By definition, all best responses produce the same expected utility, because they all maximize opponent's value.
  We can therefore do calculations of~utilities with respect to an~arbitrary best response, e.g. the standard best response retrieved by~the \emph{backward induction}.
  This method selects (recursively) the action with the maximum utility (or any of~them in~case of~multiplicity).
  ...
  For the base case, the tree containing $S$ with the minimum height is the subgame tree $S \equiv T_r$ itself, where the inequality holds by assumptions.
  ...
  Depending on the acting player in~$h$, the best response picks:
  \begin{enumerate}[(a)]
    \item the actions with $\max_{j = 1, \dots, k} \, u_i^{h_j} (\sigma)$ and $\max_{j = 1, \dots, k} \, u_i^{h_j} (\sigma^*)$ (for player~$i$)
    \item the actions with $\max_{j = 1, \dots, k} \, -u_i^{h_j} (\sigma) = \min_{j = 1, \dots, k} \, u_i^{h_j} (\sigma)$ and $\max_{j = 1, \dots, k} \, -u_i^{h_j} (\sigma^*) = \min_{j = 1, \dots, k} \, u_i^{h_j} (\sigma^*)$ (for player~$-i$)
  \end{enumerate}
