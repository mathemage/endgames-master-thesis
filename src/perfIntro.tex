...
\end{itemize}
Finally, we will outline some solution concepts for extensive-form games:
\begin{itemize}
  \item Player~$i$'s \emph{best response}~$BR _i (\sigma _{-i})$ (briefly $BR _i (\sigma)$) is a~strategy $\sigma _i \in \Sigma _i$ maximizing his expected utility against other players:
    ...
    Note that there may be several best responses, when a~node has several most valuable actions.
    Let us assume $BR _i (\sigma _{-i})$ denotes any of~them:
    this simplification is not overly harmful, since all best responses lead to the same (maximum) total utility.
\end{itemize}
