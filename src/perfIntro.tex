...
\begin{itemize}
  ...
  \item The set~$\Sigma_i$ contains all possible strategies for player~$i$.

  \item A~\emph{strategy profile} $\sigma$ is a~vector of~players' strategies:
    $\sigma = (\sigma_1, \sigma_2, \ldots, \sigma_ {\abs{P}})$.

  \item The set of~all such possible strategy profiles is denoted by~$\Sigma$.
  ...
  \item Thus, $\pi ^\sigma (h, a)$ is the (joint) probability of~taking the action~$a$ at~node~$h$.
\end{itemize}
