...
\begin{itemize}
  \item A~\emph{strategy}~$\sigma_i$ of player~$i$ gives a~probability distribution over $A(h)$ at every $h \in H$, and $\pi ^\sigma (h, a)$ is the (joint) probability of~taking the action~$a$ at~node~$h$.
