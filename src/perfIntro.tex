...
\begin{itemize}
  ...
  \item a finite set $H$ of all possible \emph{histories} (i.e. paths from the root to vertices of~the game tree),
    \begin{itemize}
...
We will also need the notion of~strategy and the expected utility:
\begin{itemize}
  \item A~(behavior) \emph{strategy}~$\sigma_i$ of player~$i$ defines a~probability distribution over actions $A(h)$ at~every node~$h \in H$, where he acts (i.e. $p(h) = i$).

  \item We use the Greek letter $\pi$ to evaluate the probability corresponding to a~strategy profile~$\sigma$:
    \[\pi ^\sigma(h) = \prod _{i \in P} \pi _i ^\sigma (h)\]

  \item $\pi ^\sigma (h, a)$ is the (joint) probability of~taking the action~$a$ at~node~$h$.
    $\Sigma_i$ denotes the set of~all such possible strategies for player~$i$.
\end{itemize}
...
\footnotetext{the paper ``An Oligopoly Model with Demand Inertia'' (\cite{Selten1968oligopoly})}
\begin{figure}[H]
  \centering
  \scriptsize
