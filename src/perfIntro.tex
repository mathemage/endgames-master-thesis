...
\begin{thm}[unexploitable subgame re-solving]
  ...
  If $u_i^r (\sigma) \le u_i^r (\sigma^*)$ (the utility improved in~the subgame), then at every node $h \in H$ also $u_i^h (\sigma) \le u_i^h (\sigma^*)$ (the utilities improved everywhere else).
\end{thm}
\begin{proof}
  \todo
\end{proof}
